...
function ItemContextProvider(_ref) {
  ...
  var _useItem = useItem(),
      _useItem$indexes = _useItem.indexes,
      parentIndexes = _useItem$indexes === void 0 ? [] : _useItem$indexes,
  ...
  var indexesRef = (0, _react.useRef)([].concat(_toConsumableArray(parentIndexes), [value.index]));
  ...
  (0, _react.useEffect)(function () {
    return selection.createPartialLayoutItem(keyRef.current, hasLazyChildren, indexesRef.current, parentKey);
  }, [selection.createPartialLayoutItem, hasLazyChildren, indexesRef, keyRef, parentKey]);
  ...
  var _useDrop = (0, _reactDnd.useDrop)({
    ...
    drop: function drop(dragItem, monitor) {
      ...
      var indexes = _toConsumableArray(item.indexes);

      var lastIndex = indexes.pop();

      switch (overPosition) {
        case TARGET_POSITION.BOTTOM:
          indexes = [].concat(_toConsumableArray(indexes), [lastIndex + 1]);
          break;

        case TARGET_POSITION.MIDDLE:
          indexes = [].concat(_toConsumableArray(indexes), [lastIndex, 0]);
          break;

        case TARGET_POSITION.TOP:
          indexes = [].concat(_toConsumableArray(indexes), [lastIndex]);
          break;
      ...
      if (onItemMove) {
        ...
        onItemMove((0, _Collection.removeItemInternalProps)(dragItem.item), tree.nodeByPath(indexes).parent);
      }
    ...
    hover: function hover(dragItem, monitor) {
      ...
      if (typeof hoverTimeoutIdRef.current !== 'number' && !expandedKeys.has(item.key)) {
      ...
      } else if (clientOffsetY > dropItemRect.bottom - dropItemRect.height * DISTANCE) {
